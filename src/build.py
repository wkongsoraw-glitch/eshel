# -*- coding: utf-8 -*-
"""일기와 통독, 두 갈래를 한 장으로 엮는다."""
import io, os, re

# ── 이름 ─────────────────────────────────────────────────────
# 사이트 이름이 정해지면 이 두 줄만 바꾸고 다시 빌드하면 된다.
사이트이름 = "에셀"
붙임말   = "하나님 만나기 · 2026년부터"
...
<style>
  /* ═══════════  하늘 토큰  ═══════════ */
  :root {
    /* 두 갈래의 색 — 일기는 짙은 물빛, 통독은 노랑 */
    --h-deep: #1D4E79;   /* 짙은 푸름 */
    --h-gold: #A97B00;   /* 노랑(글자용은 어둡게 눌러 읽히게) */
    --h-teal: #2E6E63;   /* 물빛 청록 */
    --gold-bright: #F2C94C;          /* 노랑(면·표시용) */
    --gold-wash: rgba(242,201,76,.32);  /* 형광펜 */
    --accent: var(--h-deep);
    ...
    --rule-soft: #D8E5EF;
    --code-bg:   #EDF4FA;
    --shadow:    0 1px 2px rgba(22,40,58,.05), 0 12px 30px -24px rgba(22,40,58,.45);
  ...
  /* 지금 펼친 갈래가 페이지의 색을 정한다 */
  :root[data-hue="deep"] { --accent: var(--h-deep); }
  :root[data-hue="gold"] { --accent: var(--h-gold); }
  :root[data-hue="teal"] { --accent: var(--h-teal); }

  .c-deep { --card-hue: var(--h-deep); }
  .c-gold { --card-hue: var(--h-gold); }
  .c-teal { --card-hue: var(--h-teal); }
  ...
  * { box-sizing: border-box; }
  ...
  .progress i { display: block; height: 100%; width: 0; background: var(--gold-bright); }
  ...
  .side-sub {
    ...
    color: var(--ink-faint); margin: 6px 0 24px;
  }
  ...
  .h1-orig {
    ...
    font-size: .36em; font-weight: 400; letter-spacing: .01em;
  ...
  .tree-ico { height: auto; flex: none; }
  .ico-side { width: 26px; }
  .ico-bar { width: 22px; }
  ...
  /* ═══════════  최근 기록 (표지)  ═══════════ */
  .recent { display: grid; max-width: var(--mid); border-top: 1px solid var(--rule); }
  .recent a {
    display: grid; grid-template-columns: 104px 10px minmax(0,1fr); gap: 14px; align-items: baseline;
  ...
  .recent a .d { font-family: var(--f-mono); font-size: 11.5px; color: var(--ink-faint); letter-spacing: .04em; }
  .recent a i { width: 7px; height: 7px; border-radius: 50%; background: var(--card-hue); flex: none; transform: translateY(-1px); }
  ...
  .sheet-chs a {
    display: grid; grid-template-columns: 58px minmax(0,1fr); gap: 12px; align-items: baseline;
  ...
  .chapter h3::before { content: ""; width: 6px; height: 6px; flex: none; background: var(--gold-bright); transform: translateY(-3px); }
  ...
  /* 형광펜 — 가장 강조하고 싶은 말에만 아껴 쓴다 */
  mark {
    background: linear-gradient(transparent 55%, var(--gold-wash) 55%);
  ...
  .verse {
    ...
    padding: 20px 24px; margin: 0 0 26px; max-width: var(--mid);
    box-shadow: var(--shadow);
  }
  .verse p {
    font-family: var(--f-display); font-size: 18px; line-height: 1.9;
    margin: 0; max-width: none;
  }
  .verse .ref {
    display: block; font-family: var(--f-mono); font-size: 11px; letter-spacing: .1em;
    color: var(--h-gold); margin-top: 12px;
  }

  /* ═══════════  필사 사진 카드  ═══════════ */
  .copies {
    display: grid; gap: 16px; grid-template-columns: 1fr;
    max-width: var(--wide); margin: 44px 0 0;
  }
  @media (min-width: 700px) { .copies { grid-template-columns: 1fr 1fr; } }
  figure.copy {
    margin: 0; background: var(--surface);
    border: 1px solid var(--rule); border-top: 3px solid var(--h-teal);
    border-radius: 4px; padding: 10px 10px 6px; box-shadow: var(--shadow);
  }
  figure.copy img { display: block; width: 100%; height: auto; border-radius: 2px; }
  figure.copy figcaption {
    display: flex; justify-content: space-between; align-items: baseline; gap: 12px;
    padding: 11px 4px 5px;
  }
  figure.copy .d { font-family: var(--f-mono); font-size: 11px; color: var(--ink-faint); }
  figure.copy .src { font-size: 13.5px; font-weight: 600; color: var(--h-teal); text-align: right; }
  ...
  /* ═══════════  표  ═══════════ */
  .table-wrap { overflow-x: auto; margin: 0 0 28px; max-width: var(--wide); border-top: 1px solid var(--rule); }
  ...
  /* ═══════════  통독 진행  ═══════════ */
  .reading {
    display: flex; flex-wrap: wrap; gap: 8px; margin: 0 0 26px; max-width: var(--wide);
  }
  .reading span {
...
<header class="topbar">
  <div class="topbar-row">
    <a class="topbar-title" href="#cover"><img class="tree-ico ico-bar" src="tree.webp" alt=""><span class="topbar-name">__TITLE__</span></a>
    <button class="toc-btn" type="button" aria-expanded="false" aria-controls="sheet"><span class="bars"></span>차례</button>
  </div>
  <nav class="chips" aria-label="갈래">
    <a href="#cover" data-vol="v0" class="c-deep"><i></i>표지</a>
    <a href="#v1" data-vol="v1" class="c-deep"><i></i>감사</a>
    <a href="#v2" data-vol="v2" class="c-gold"><i></i>통독</a>
    <a href="#v3" data-vol="v3" class="c-teal"><i></i>필사</a>
  </nav>
</header>
...
<div class="sheet" id="sheet">
  ...
  <div class="sheet-body">
    <div class="sheet-label">갈래 고르기</div>
    ...
    <div class="sheet-label sheet-chs-label">이 갈래의 기록</div>
...
<aside class="side">
  <div class="side-inner">
    <a class="side-title" href="#cover"><img class="tree-ico ico-side" src="tree.webp" alt="">__TITLE__</a>
    <div class="side-sub">__SUB__</div>
    <nav class="vol-nav" aria-label="갈래">
      <a href="#cover" data-vol="v0" class="c-deep"><i></i><span>표지</span></a>
      <a href="#v1" data-vol="v1" class="c-deep"><i></i><span>감사</span></a>
      <a href="#v2" data-vol="v2" class="c-gold"><i></i><span>통독</span></a>
      <a href="#v3" data-vol="v3" class="c-teal"><i></i><span>필사</span></a>
    </nav>
    <nav class="ch-nav" aria-label="이 갈래의 기록"></nav>
  </div>
</aside>
...
HUB = r'''<section class="vol" id="v0" data-hue="deep">

  <header class="cover">
    <div class="eyebrow">__SUB__</div>
    <h1>__TITLE__ <span class="h1-orig">(<bdi>אשל</bdi> · eshel)</span></h1>
    <p class="lede">에셀은 아브라함이 브엘세바에 심은 나무입니다. 하나님을 만나기 시작한 2026년 여름, 그 마음으로 여기에 기록을 심어 봅니다.</p>
    <div class="verse">
      <p>아브라함은 브엘세바에 에셀나무를 심고 거기서 영원하신 여호와의 이름을 불렀으며</p>
      <span class="ref">창세기 21:33</span>
    </div>
    <div class="cover-meta">
      <div><b>시작</b> 2026년 8월 31일</div>
      <div><b>지금 읽는 책</b> 요한복음</div>
    </div>
  </header>

  <div class="cards">
    <a class="card c-deep" href="#v1" data-vol="v1">
      <h3>감사</h3>
      <p>하루에서 받은 것을 하나씩 세어 적습니다.</p>
      <div class="meta">최근 기록이 맨 위에 온다</div>
    </a>
    <a class="card c-gold" href="#v2" data-vol="v2">
      <h3>통독</h3>
      <p>말씀을 등불 삼아 한 장씩 걸어갑니다. 요한복음부터 읽습니다.</p>
      <div class="meta">요한복음 · 전체 21장</div>
    </a>
    <a class="card c-teal" href="#v3" data-vol="v3">
      <h3>필사</h3>
      <p>말씀을 손으로 옮겨 적고, 사진으로 남깁니다.</p>
      <div class="meta">적은 종이가 그대로 기록이 된다</div>
    </a>
  </div>

  <section class="chapter" id="v0recent">
    <div class="chapter-body">
      <div class="chapter-eyebrow">최근 기록</div>
      <h2>날짜순으로</h2>
...
(function () {
  ...
  function show(id, keepScroll) {
    ...
    root.setAttribute("data-hue", vol.getAttribute("data-hue") || "deep");
...
# ── 두 갈래를 읽어 엮는다 ──────────────────────────────────
frags = {}
for name in ("일기.html", "통독.html", "필사.html"):
    frags[name] = io.open(os.path.join(HERE, name), encoding="utf-8").read()

# 표지의 '최근 기록' — 두 갈래에서 날짜 달린 기록을 모아 최신순 열 개
entries = []
for name, cls in (("일기.html", "c-deep"), ("통독.html", "c-gold"), ("필사.html", "c-teal")):
    # 주석 속 틀은 세지 않는다
    scan = re.sub(r"<!--.*?-->", "", frags[name], flags=re.S)
    for m in re.finditer(
        r'<section class="chapter" id="([^"]+)" data-date="([^"]+)"[^>]*>.*?<h2>(.*?)</h2>',
        scan, re.S):
        entries.append((m.group(2), m.group(1), re.sub(r"<[^>]+>", "", m.group(3)).strip(), cls))
    for m in re.finditer(
        r'<figure class="copy" id="([^"]+)" data-date="([^"]+)".*?<span class="src">(.*?)</span>',
        scan, re.S):
        entries.append((m.group(2), m.group(1), "필사 · " + re.sub(r"<[^>]+>", "", m.group(3)).strip(), cls))
entries.sort(key=lambda e: e[0], reverse=True)
rows = "".join(
    '\n        <a href="#%s" class="%s"><span class="d">%s</span><i></i><span class="t">%s</span></a>'
    % (eid, cls, date.replace("-", "."), title)
    for date, eid, title, cls in entries[:10])
if not rows:
    rows = '\n        <p style="border:0">아직 기록이 없습니다.</p>'

parts = [HEAD, HUB.replace("<!--RECENT-->", rows + "\n      "),
         frags["일기.html"], frags["통독.html"], frags["필사.html"], TAIL]
html = "\n".join(parts)
html = html.replace("__TITLE__", 사이트이름).replace("__SUB__", 붙임말)

DESC = "하나님을 만나며 남기는 기록. 감사와 통독과 필사."
...
<meta property="og:image" content="https://wkongsoraw-glitch.github.io/eshel/og.jpg">
...
""" % (DESC, 사이트이름, DESC, html[:cut], html[cut:].strip())
